...
// Function to generate the Google Auth URL with correct SCOPES
const getGoogleAuthUrl = (req, res) => {
  const scopes = [
    "https://www.googleapis.com/auth/userinfo.profile",
    "https://www.googleapis.com/auth/userinfo.email",
    "https://www.googleapis.com/auth/calendar", // Full access to read/write
  ];

  const url = oauth2Client.generateAuthUrl({
    access_type: "offline", // Essential for getting the refresh_token
    scope: scopes,
    prompt: "consent", // Forces Google to show the permission tick-boxes again
  });
...
const deleteGoogleEvent = async (user, eventId) => {
  try {
    oauth2Client.setCredentials({
      access_token: user.googleTokens.accessToken,
      refresh_token: user.googleTokens.refreshToken,
