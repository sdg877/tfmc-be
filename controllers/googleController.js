...
exports.connectGoogleCalendar = async (req, res) => {
  try {
    const { code } = req.body;
    if (!code) return res.status(400).json({ message: "No authorisation code provided" });
...
exports.addGoogleEvent = async (req, res) => {
  try {
    ...
    oauth2Client.setCredentials({ refresh_token: user.googleTokens.refreshToken });
...
exports.getDailyEnergyUsage = async (req, res) => {
  try {
    const user = await User.findById(req.user.id);
    if (!user || !user.googleConnected) return res.json({ googleEnergyDrain: 0 });
    ...
    const mappings = user.calendarMapping?.length > 0 ? user.calendarMapping : [
...
exports.disconnectGoogle = async (req, res) => {
  try {
    await User.findByIdAndUpdate(req.user.id, { googleConnected: false, googleTokens: {} });
